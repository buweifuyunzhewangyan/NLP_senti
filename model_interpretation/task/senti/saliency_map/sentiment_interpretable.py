...
def get_args():
    ...
    parser.add_argument(
        '--init_checkpoint',
        type=str,
        default='../rnn/checkpoints/final.pdparams',
    ...
    parser.add_argument(
        '--output_dir',
        type=Path,
        default='../output/senti_ch.lstm',
        help='interpretable output directory')
    parser.add_argument('--start_id', type=int, default=0)
    parser.add_argument('--vocab_path', type=str, default='../rnn/vocab.txt')
    parser.add_argument(
        '--language',
        type=str,
        default='ch',
...
def extract_integrated_gradient_scores(
        ...
        model, result, pred_label, err_total, offset, out_handle,result_name):
    embedded_grads_list = []
    ...
    if args.base_model.startswith('roberta'):
        ...
        result['char_attri'] = collections.OrderedDict()
        for token_id, token_info in sorted(
                #changed by LYS 加了一个abs
                char_attribution_dict.items(), key=lambda x: abs(x[1][1]),
                reverse=True):
            result['char_attri'][token_id] = token_info

    out_handle.write(json.dumps(result, ensure_ascii=False) + '\n')
    # added by LYS 根据score，筛选字符排序
    id,pred_label,char_list = get_INFO_fromResult(result,score_limit=0.06)
    # add by LYS 输出结果文件
    with open(result_name,'a') as f:
        f.write(id + "\t")
        f.write(pred_label + "\t")
        f.write(char_list[0:-1] + "\n")
...
def get_INFO_fromResult(result,score_limit):
    ...
    char_list = ''
    for j,char_key in enumerate(char_attri):
        score = char_attri[char_key][1]
        if abs(score)<score_limit:
            break
        char_list+=str(char_key)+','
    return id,pred_label,char_list
...
if __name__ == "__main__":
    ...
    with paddle.amp.auto_cast(enable=args.use_amp), \
        ...
        #将结果打包
        with zipfile.ZipFile(result_name + '.zip', mode='w', compression=zipfile.ZIP_DEFLATED) as zf:
            zf.write(filename=result_name)

...
def get_args():
    ...
    parser.add_argument(
        '--from_pretrained',
        type=str,
        default='data/skep_ernie_1.0_large_ch',
    ...
    parser.add_argument(
        '--data_dir',
        type=str,
        default='../../../data/similarity_ch',
        help='data directory includes train / develop data')
    parser.add_argument('--eval',default='@', action='store_true')
    parser.add_argument(
        '--init_checkpoint',
        type=str,
        default='../simnet/checkpoints/final.pdparams',
    ...
    parser.add_argument(
        '--output_dir',
        type=Path,
        default='../output/similarity_ch.lstm',
...
def init_lstm_var(args):
    if args.language == 'ch':
        vocab = Vocab.load_vocabulary(
            "../simnet/vocab.char", unk_token='[UNK]', pad_token='[PAD]')
    else:
        vocab = Vocab.load_vocabulary(
...
def extract_attention_scores(args, result, atts, q_tokens, t_tokens, out_handle,
    ...
    if args.base_model.startswith('roberta'):
        ...
        t_inter_score = inter_score[0][SEP_idx + add_idx:
...
def IG_roberta_inter_score(
        ...
        pred_confidence, baseline_pred_confidence, SEP_idx, add_idx, err_total):
